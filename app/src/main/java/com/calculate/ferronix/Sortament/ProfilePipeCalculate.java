...
import Control.NetworkHelper; // Ensure this class exists and works correctly

public class ProfilePipeCalculate extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.profile_pipe_calculate);
        ...
        textViewUnit.setText(R.string.unit_meter); // Display "м" for unit
    ...
    private void initializeViews() {
        ...
        editTextLength = findViewById(R.id.editTextLengthProfilePipeW); // This EditText's purpose changes (length or weight input)
        editTextSideA = findViewById(R.id.editTextSideA); // External Width
        editTextSideB = findViewById(R.id.editTextSideB); // External Height
        editTextWall = findViewById(R.id.editTextWallProfilePipeW); // Wall Thickness
        ...
        textViewUnit = findViewById(R.id.textViewUnit); // TextView for unit "м" or "кг"
        ...
        if (editTextDensity == null || editTextLength == null || editTextSideA == null ||
                ...
                textViewUnit == null || btnCalculate == null || btnMaterial == null ||
                btnMark == null || btnGoWeight == null || btnGoLength == null) {
    ...
    private void setupListeners() {
        ...
        findViewById(R.id.btnCalculateProfilePipeW).setOnClickListener(v -> {
        ...
        btnGoWeight.setOnClickListener(v -> {
            ...
            textViewUnit.setText(R.string.unit_meter); // Change unit to "м"
    ...
    private void showGradeMenu(String material) {
        ...
        if (material.equals(getString(R.string.material_black_metal))) {
            ...
            Log.e("ProfilePipeCalculate", getString(R.string.error_unknown_material) + ": " + material);
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            ...
            if (index != -1 && index < densities.length) {
                ...
                Log.e("ProfilePipeCalculate", getString(R.string.log_density_error) + ": Grade=" + grade + ", Material=" + material);
    ...
    // Common input validation method for Profile Pipe dimensions
    private boolean validateInputs(String densityStr, String mainInputStr, String sideAStr, String sideBStr, String wallStr) {
        ...
        try {
            ...
            double externalWidth = Double.parseDouble(sideAStr);
            double externalHeight = Double.parseDouble(sideBStr);
            double wallThickness = Double.parseDouble(wallStr);

            // Check for non-positive values
            if (density <= 0 || mainValue <= 0 || externalWidth <= 0 || externalHeight <= 0 || wallThickness <= 0) {
                totalWeight.setText(R.string.error_negative_values);
                return false;
            }

            // Specific validation for profile pipe dimensions
            // Wall thickness should not be greater than or equal to half of the smallest outer dimension
            if (2 * wallThickness >= externalWidth || 2 * wallThickness >= externalHeight) {
                totalWeight.setText(R.string.error_wall_thickness_too_large); // New string resource
                return false;
            }
            ...
            Log.e("ProfilePipeCalculate", getString(R.string.log_parsing_error) + e.getMessage());
            return false; // Validation failed due to number format
        }
    }

    // Helper method to calculate the cross-sectional area of the profile pipe material in cm²
    private double calculateMaterialCrossSectionAreaCm2(double externalWidth_MM, double externalHeight_MM, double wallThickness_MM) {
        double externalWidthCm = externalWidth_MM * MM_TO_CM;
        double externalHeightCm = externalHeight_MM * MM_TO_CM;
        double wallThicknessCm = wallThickness_MM * MM_TO_CM;

        // Calculate internal dimensions
        double internalWidthCm = externalWidthCm - 2 * wallThicknessCm;
        double internalHeightCm = externalHeightCm - 2 * wallThicknessCm;

        // Area of the material (external rectangle area - internal hollow area)
        return (externalWidthCm * externalHeightCm) - (internalWidthCm * internalHeightCm);
    }
    ...
    private void calculateWeightOutput() {
        ...
        String sideAStr = editTextSideA.getText().toString().trim(); // External Width (A)
        String sideBStr = editTextSideB.getText().toString().trim(); // External Height (B)
        String wallStr = editTextWall.getText().toString().trim(); // Wall Thickness
        String pricePerKgStr = editTextPricePerKg.getText().toString().trim();
        String quantityStr = editTextQuantity.getText().toString().trim();

        // Validate inputs
        if (!validateInputs(densityStr, lengthInputStr, sideAStr, sideBStr, wallStr)) {
        ...
        try {
            ...
            double externalWidth_MM = Double.parseDouble(sideAStr);
            double externalHeight_MM = Double.parseDouble(sideBStr);
            double wallThickness_MM = Double.parseDouble(wallStr);

            double densityKgPerCm3 = density * G_PER_CM3_TO_KG_PER_CM3; // Convert density to kg/cm³

            // Calculate material cross-sectional area in cm²
            double materialAreaCm2 = calculateMaterialCrossSectionAreaCm2(externalWidth_MM, externalHeight_MM, wallThickness_MM);

            // Calculate total volume for one piece (cm³)
            double volumeCm3 = materialAreaCm2 * (lengthInputMeters * 100); // Length from meters to cm
            ...
            if (!pricePerKgStr.isEmpty()) {
                try {
                    ...
                    double pricePerUnit = totalCost / totalQuantity; // Cost per single piece
            ...
            totalWeight.setText(result.toString()); // Display the final result
    ...
    private void calculateLengthOutput() {
        ...
        // Validate inputs
        if (!validateInputs(densityStr, weightInputStr, sideAStr, sideBStr, wallStr)) {
            return; // Exit if validation fails
        }
        ...
        try {
            ...
            // Calculate total volume from input weight (cm³)
            double volumeCm3 = weightInputKg / densityKgPerCm3;

            // Calculate length per piece (meters)
            double lengthPerPiece = volumeCm3 / materialAreaCm2 / 100; // Convert cm to meters
    ...
    private void sendAnalytics(String calculationType) {
        ...
        analytics.put(getString(R.string.analytics_key_template), getString(R.string.analytics_template_profile_pipe)); // Use the new string resource
