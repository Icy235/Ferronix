...
public class PryamougolniyProfileCalculate extends AppCompatActivity {
    ...
    private void calculateWeight() {
        try {
            ...
            String sideBStr = editTextSideB.getText().toString().trim(); // Новое поле для стороны B
            String pricePerKgStr = editTextPricePerKg.getText().toString().trim();
            String quantityStr = editTextQuantity.getText().toString().trim();

            if (densityStr.isEmpty() || weightStr.isEmpty() || sideAStr.isEmpty() || sideBStr.isEmpty()) {
            ...
            if (density <= 0 || weight <= 0 || sideA <= 0 || sideB <= 0) {
            ...
            // Площадь поперечного сечения (прямоугольник)
            double area = sideACm * sideBCm; // площадь в см²
            ...
            analytics.put("Шаблон", "Прямоугольный профиль");
    ...
    private void calculateLength() {
        try {
            ...
            if (densityStr.isEmpty() || lengthStr.isEmpty() || sideAStr.isEmpty() || sideBStr.isEmpty()) {
                total.setText("Заполните все поля!");
                return;
            }
            ...
            // Проверка положительных значений
            if (density <= 0 || length <= 0 || sideA <= 0 || sideB <= 0) {
                total.setText("Значения должны быть > 0");
                return;
            }
